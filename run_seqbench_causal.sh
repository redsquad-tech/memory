...
mode="${1:-pilot}"
data="${2:-$seqroot/data/causal-v1}"
output="${3:-$root/runs/seqbench-causal-v1/$mode}"
...
for file in \
  ...
  clutrr-full.csv clutrr-nonce.csv clutrr-counterfactual.csv clutrr-noise.csv \
  proofwriter-full.csv proofwriter-supervision.csv \
    ...
    if [ -f "$destination/metrics.json" ]; then
      ...
      (
        ...
        "$seqroot/.venv/bin/seqbench" run \
          "$seqroot/specs/runs/causal_v1.yaml" \
...
if [ -e "$comparison" ]; then
  mv "$comparison" "$comparison.previous-$(date +%s)"
fi
# shellcheck disable=SC2086
