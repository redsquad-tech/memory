...
class CategoricalMemoryConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.num_classes > torch.iinfo(torch.int16).max:
            raise ValueError("categorical mixture supports at most 32767 classes")
...
class CategoricalAssociativeMemory:
    def __init__(
        ...
        *,
        ...
        self.birth_label = torch.full((config.capacity,), -1, dtype=torch.int16, device=self.device)
    ...
    def _update_usage(self, read: CategoricalReadResult) -> None:
        with torch.no_grad():
        ...
        *,
        ...
        try:
            ...
            with torch.no_grad():
                ...
                self.base_counts[:] = snapshot[6]
            self.size, self.step, self.revision, self.next_atom_uid = snapshot[7:]
            raise
...
class CategoricalMemoryOnlineModel:
    def __init__(self, memory: CategoricalAssociativeMemory):
        self.memory = memory

    def predict(self, query: TernaryQuery, *, prior_mode: PriorMode | None = None) -> Prediction:
        read = self.memory.read(query, prior_mode=prior_mode)
        return Prediction(read.probabilities, int(read.prediction), read)
    ...
    def observe(
        ...
        *,
        ...
        return self.memory.observe(
            query,
            target,
            pre_read=prediction.token,  # type: ignore[arg-type]
